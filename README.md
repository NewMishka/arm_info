...
Ветка разработки содержит изменения, подготовленные для следующего выпуска **1.3.0**. До завершения полевого чек-листа и отдельного решения о выпуске `VERSION`, RPM spec и встроенная версия скрипта намеренно остаются `1.2.4`.
...
Полный список и статус проверки: [черновик release notes 1.3.0](docs/releases/v1.3.0.md) и [pre-release checklist](docs/PRE_RELEASE_CHECKLIST.md).
...
- [Черновик release notes 1.3.0](docs/releases/v1.3.0.md)
...
Документация кандидата **1.3.0** подготовлена для проверки; номер в `VERSION`, скрипте и RPM spec будет изменён только непосредственно перед выпуском.
