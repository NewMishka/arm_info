...
В **1.2.2** текстовый корпоративный отчёт выровнен по колонкам и аккуратно переносит длинные значения. Блок `802.1X` показывает активный профиль, EAP-метод, пути клиентского/CA-сертификата, даты действия и готовую проверку `openssl x509 -in "<CERT>" -noout -dates`. Перед интерактивным TXT-выводом экран очищается; JSON остаётся чистым. Команды в рекомендациях сопровождаются пояснением в скобках, что именно даст каждая команда. Shell pipeline и регулярные выражения с `|` сохраняются как одна команда.

Стандартный анализ также использует выровненные рекомендации. Температура CPU в пользовательском отчёте выводится как медиана (`NN°C (медиана)`) без одновременного значения максимума.
...
`arm_info 1.2.3` содержит встроенные профили для типовых проблем корпоративных АРМ РЕД ОС. Они **не смешиваются с аппаратным health score** и выводят самостоятельные статусы `OK/WARN/CRIT/N/A`.
...
CI выполняет `bash -n`, ShellCheck уровня error, базовые CLI/JSON/privacy tests, enterprise tests, compatibility syntax, installer smoke, документационный contract и RPM build smoke test.
