...
Активные проверки DC, CIFS и GIO имеют общий бюджет 30 секунд; TCP-проверки DC выполняются максимум по четыре одновременно. Все обнаруженные ресурсы остаются в отчёте, а не уложившиеся в бюджет получают N/A. Для крупных инфраструктур: `sudo arm_info -c -p --network-budget 60 --network-jobs 6`.
