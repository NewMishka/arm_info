...
- Убраны оставшиеся служебные формулировки «кандидат 1.3» после публикации релиза.

## 1.3.0 — 2026-09-18

Релиз расширяет корпоративную диагностику, сохраняя single-file поставку и совместимость форматов TXT/JSON. Подробности и границы подтверждённой проверки: `docs/releases/v1.3.0.md`.

### Added and changed
- Третий этап архитектурного рефакторинга: DNS/DC и сетевые ресурсы проходят конвейер `collect → normalized inventory → probe → checks → output`. CIFS, autofs и GVFS/GIO хранят раздельные признаки «настроен», «смонтирован» и «доступен»; дубли объединяются, каждый DC и ресурс остаётся отдельной строкой, single-file поставка и TXT/JSON-контракт сохранены.
- Добавлен `tests/test_stage3_architecture.sh`: контролирует границу collectors/checks, объединение CIFS/autofs и FUSE/GIO, пассивность autofs, отдельные статусы неисправных ресурсов и privacy. Полный тестовый прогон и сетевой профиль не показали ухудшения скорости относительно базы этапа 2.
- Добавлен профиль `mail` и раздел `ПОЧТА / MAIL` в корпоративном отчёте: универсальное обнаружение IMAP/SMTP из Thunderbird-совместимых `prefs.js` или `--mail-endpoint`, ограниченные общим бюджетом DNS/TCP/TLS/STARTTLS-проверки, срок/hostname/цепочка сертификата, AUTH capabilities, MX/SRV и privacy. Аутентификация, чтение ящика, изменение Kerberos cache и отправка писем не выполняются.
...
- Активные проверки DC/CIFS/autofs/GIO/mail используют общий бюджет времени. После его исчерпания ресурсы остаются в отчёте со статусом N/A и причиной `лимит времени`.
...
- Исправлено ложное размножение GVFS-ресурсов: обычные подпапки открытой SMB-шары больше не принимаются за отдельные mounts; FUSE-collector принимает только mount descriptors, а GIO URI нормализуются до корня `server/share`.
- Вложенные CIFS mounts, создаваемые DFS referrals внутри одной корневой шары, больше не выводятся как самостоятельные сетевые диски; в диагностике корневого ресурса сохраняется их количество.
