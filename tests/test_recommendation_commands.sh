...
grep -Fq 'timeout -k 1 6 ls -U -A -1 -- "$mnt/"' "$SCRIPT" || fail 'CIFS full-directory readdir probe missing'
grep -Fq 'timeout -k 1 6 stat -L -- "$sample"' "$SCRIPT" || fail 'CIFS sample metadata probe missing'
