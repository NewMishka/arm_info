...
- `tests/test_stage4_base_pipeline.sh` — конвейер стандартного отчёта collectors → normalized snapshot → checks/scoring → TXT/JSON, отсутствие рекомендаций в collector, отсутствие повторных probes в emitters и сохранение дисковых рекомендаций;
...
- архитектурный контракт стандартного отчёта collectors → snapshot → checks/scoring → один TXT/JSON emitter;
