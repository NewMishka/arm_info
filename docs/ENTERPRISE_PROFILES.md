...
- доступность найденных контроллеров по TCP 88/389; каждый найденный DC выводится отдельной строкой `Контроллер #N` со статусом Kerberos 88 и LDAP 389, после чего сохраняется общий итог TCP-проверок.
