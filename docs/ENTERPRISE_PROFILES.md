...
Начиная с **1.2.2**, TXT-отчёт enterprise-профилей выводится в выровненных колонках, длинные значения и рекомендации переносятся по ширине терминала. Для каждой команды в рекомендациях в скобках выводится краткое объяснение результата команды. Shell pipeline и `|` внутри регулярных выражений сохраняются как части одной команды и не разбиваются на отдельные псевдокоманды. При интерактивном TXT-запуске корпоративного профиля экран предварительно очищается; JSON не загрязняется управляющими последовательностями.

`--corp` и полная форма `--profile enterprise` теперь **сохраняют отчёт по умолчанию**. Без `-o` файл создаётся в текущем доступном для записи каталоге, при необходимости используется `/tmp`. Для TXT имя имеет вид `ARM_INFO_CORP_<HOST>_<DATE>.txt`, а в privacy-режиме — `ARM_INFO_CORP_PRIVATE_<DATE>.txt`. Ключ `--no-save` отключает сохранение. Через `-o FILE` можно задать точный путь, а через `-o DIR` — каталог для автоматически сформированного имени.
...
- FQDN и доменные SRV;
- активные NetworkManager 802.1X-профили и EAP-метод;
...
Первый вариант одновременно выводит корпоративный TXT-отчёт и автоматически сохраняет его. Для запуска только на экран:
