...
- [ ] Mail-профиль не принимает URI с credentials, не выполняет аутентификацию/отправку, использует общий network budget и скрывает server/domain/Subject/Issuer в privacy.
- [ ] RAID-рекомендация не предполагает фиксированное `/dev/md0`, а требует фактический `MD_DEVICE`.
- [ ] Все режимы по умолчанию только выводят отчёт в терминал; `-s/--save` явно включает сохранение; `-o/--output` без `--save` отклоняется с кодом 64; строки о сохранении отсутствуют при обычном запуске.
